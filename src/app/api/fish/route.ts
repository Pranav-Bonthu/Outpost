...
import { pickRandomFish } from "@/lib/wildlife/fish";
...
export async function POST() {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const species = pickRandomFish();

      const existing = await tx.fishCatch.findUnique({
    ...
    if (err instanceof Error && err.message === "INSUFFICIENT_FUNDS") {
      return NextResponse.json(
        { error: "Not enough money to go fishing yet." },
        { status: 400 }
      );
    }
